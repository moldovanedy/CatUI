...
/// <summary>
/// A platform-specific "save file dialog".
/// </summary>
public class NativeSaveFileDialog : NativeFileDialogBase
{
    ...
    /// <summary>
    /// The suggested file name for saving. Can be empty.
    /// </summary>
    ...
    /// <returns>
    /// Returns the selection in a <see cref="IFilePicker.SaveFileResponse"/> if the user successfully selected
    ...
    /// </exception>
    public async Task<IFilePicker.SaveFileResponse?> OpenAsync(CancellationToken? cancellationToken = null)
    {
        ...
        try
        {
            return await OS.FilePicker.SaveFileAsync(
    ...
    /// Something went wrong while the user was picking files.
    /// </exception>
    public IFilePicker.SaveFileResponse? Open()
    {
